...
"""
For 1 X 1 Convolution
"""


class OneOneConvolution(nn.Module):

    def __init__(self, in_ch, out_ch, use_bias):
        super(OneOneConvolution, self).__init__()
        self.input = nn.Conv2d(in_ch, out_ch, kernel_size=1, stride=1, padding=0, bias=use_bias)

        self.one_one = nn.Sequential(nn.ReLU(True),
...
class DownDiscriminator(nn.Module):
    def __init__(self, in_ch, out_ch, use_bias, norm_layer):
        ...
        if norm_layer == 'Spectral':
            self.down = nn.Sequential(SpectralConvolution(in_ch, out_ch, w=4, s=2, p=1, bias=use_bias),
                                      nn.LeakyReLU(0.2, True))
        else:
            self.down = nn.Sequential(NaiveConvolution(in_ch, out_ch,
                                                       w=4, s=2, p=1, bias=use_bias, norm_layer=norm_layer),
                                      nn.LeakyReLU(0.2, True))
...
class KeepDiscriminator(nn.Module):
    def __init__(self, in_ch, out_ch, use_bias, norm_layer):
        super(KeepDiscriminator, self).__init__()
        if norm_layer == 'Spectral':
            self.down = nn.Sequential(SpectralConvolution(in_ch, out_ch, w=4, s=1, p=1, bias=use_bias),
                                      nn.LeakyReLU(0.2, True))
        else:
            self.down = nn.Sequential(NaiveConvolution(in_ch, out_ch,
                                                       w=4, s=1, p=1, bias=use_bias, norm_layer=norm_layer),
                                      nn.LeakyReLU(0.2, True))
