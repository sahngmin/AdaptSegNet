...
def main():
    ...
    else:  # training model from pre-trained DeepLab on source & previous target domains
    ...
    for i_iter in range(args.num_steps):
        ...
        if not args.from_scratch and args.dm:
        ...
        if args.dm:
            pred_target2, pred_target1, _, _ = model(images_target, input_size_target)
        else:
            _, _, pred_target2, pred_target1 = model(images_target, input_size_target)

        if args.gan == 'Hinge':
            loss_adv1 = adversarial_loss_1(F.softmax(pred_target1, dim=1), generator=True)
            loss_adv2 = adversarial_loss_2(F.softmax(pred_target2, dim=1), generator=True)
        elif args.gan == 'new_Hinge':
            loss_adv1 = adversarial_loss_1(F.softmax(pred_target1, dim=1), real_samples=F.softmax(pred1, dim=1),
                                                  generator=True, new_Hinge=True)
            loss_adv2 = adversarial_loss_2(F.softmax(pred_target2, dim=1), real_samples=F.softmax(pred2, dim=1),
                                                  generator=True, new_Hinge=True)
        else:
        ...
        if args.gan == 'Hinge' or args.gan == 'new_Hinge':
            loss_D1 = adversarial_loss_1(F.softmax(pred_target1, dim=1),
                                         real_samples=F.softmax(pred1, dim=1), generator=False)
            loss_D2 = adversarial_loss_2(F.softmax(pred_target2, dim=1),
                                         real_samples=F.softmax(pred2, dim=1), generator=False)
